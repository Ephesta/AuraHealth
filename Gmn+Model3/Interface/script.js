...
function requestAirPollutionInfo(city) {
    ...
    fetch(`${BACKEND_API_URL}/get_air_pollution?city=${encodeURIComponent(city)}`)
    .then(response => response.json())
    .then(data => {
        if (data.error) {
            infoTxt.classList.replace("pending", "error");
...
function requestEarthquakeInfo() {
    ...
    fetch(`${BACKEND_API_URL}/get_earthquake_data`)
    .then(response => response.json())
    .then(data => {
        if (data.error) {
            infoTxt.classList.replace("pending", "error");
...
function requestWeatherInfo(city) {
    ...
    // Konum bilgisini de gönder
    const weatherData = { city: city };
    if (currentCoords) {
        weatherData.lat = currentCoords.lat;
        weatherData.lon = currentCoords.lon;
    }
    ...
    .then(data => {
        if (data.error) {
            ...
            addMessage("Üzgünüm, hava durumu bilgisi bulunamıyor. Lütfen başka bir şehir deneyin.", 'bot');
        } else {
            ...
            // Chat geçmişini temizle ve ilk mesajı ekle
            if (chatMessages.children.length === 0) {
                chatMessages.innerHTML = ''; // Sohbeti temizle
                addMessage(`Selam! Ben senin kişisel asistanınım. İstersen sohbet edelim, istersen dertleşelim.`, 'bot');
                addMessage(`❓ Yardım Komutları:\n"!help", "help" veya "?" yardım menüsünü gösterir`, 'bot');
            }
        }
    })
    .catch(error => {
        ...
        currentCity = ""; // Hata durumunda şehri sıfırla
        chatMessages.innerHTML = ''; // Sohbeti temizle
        addMessage("Sunucuya bağlanılamadı. Lütfen sunucunun çalıştığından emin olun.", 'bot');
    });
}
...
function showPanel(panelName) {
    ...
    earthquakePanel.classList.remove("active-panel");
    wrapper.classList.remove("earthquake-active");
    ...
    } else if (panelName === 'air-pollution') {
...
function sendMessage() {
    ...
    // Konum al ve mesajla birlikte gönder
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
            (position) => {
                // Konum ile birlikte gönder
                sendChatWithLocation(message, position.coords.latitude, position.coords.longitude);
            },
            (error) => {
                // Konum alınamazsa currentCoords'u kullan
                if (currentCoords) {
                    sendChatWithLocation(message, currentCoords.lat, currentCoords.lon);
                } else {
                    sendChatWithLocation(message, null, null);
                }
            }
        );
    } else {
        // Geolocation desteklenmiyorsa currentCoords'u kullan
        if (currentCoords) {
            sendChatWithLocation(message, currentCoords.lat, currentCoords.lon);
        } else {
            sendChatWithLocation(message, null, null);
...
function sendChatWithLocation(message, lat, lon) {
    ...
    .then(data => {
        if (data.gemini_response) {
            addMessage(data.gemini_response, 'bot');
        } else if (data.error) {
...
async function getGeminiResponse(userMessage, city) {
    addMessage("Asistan yazıyor...", 'bot');
    ...
    try {
        ...
        if (data.error) {
            addMessage("Üzgünüm, isteğinizi işlerken bir hata oluştu: " + data.error, 'bot');
        } else {
            addMessage(data.gemini_response, 'bot');
        }
    } catch (error) {
        ...
        addMessage("Bağlantı hatası: Sunucuya ulaşılamadı veya bir sorun oluştu.", 'bot');
